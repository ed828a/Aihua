...
import com.dew.aihua.repository.database.stream.model.StreamEntity.Companion.STREAM_SERVICE_ID
import com.dew.aihua.repository.database.stream.model.StreamEntity.Companion.STREAM_TABLE
import com.dew.aihua.repository.database.stream.model.StreamEntity.Companion.STREAM_URL
...
@Entity(tableName = STREAM_TABLE, indices = [Index(value = arrayOf(STREAM_SERVICE_ID, STREAM_URL), unique = true)])
class StreamEntity(
    @field:ColumnInfo(name = STREAM_SERVICE_ID) var serviceId: Int = Constants.NO_SERVICE_ID,
    ...
    @field:ColumnInfo(name = STREAM_DURATION) var duration: Long?) : Serializable {
