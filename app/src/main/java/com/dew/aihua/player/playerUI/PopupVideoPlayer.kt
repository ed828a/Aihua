...
class PopupVideoPlayer : Service() {
    ...
    private fun initPopup() {
        ...
        tossFlingVelocity = PlayerHelper.getTossFlingVelocity()
    ...
    private fun createNotification(): NotificationCompat.Builder {
        ...
        val intent = PlayerNavigationHelper.getPopupPlayerActivityIntent(this)
    ...
    private fun updatePopupSize(width: Int, height: Int) {
        ...
        Log.d(TAG, "updatePopupSize() called with: width = [$width1], height = [$height1]")

        width1 = (when {
            width1 > maximumWidth -> maximumWidth.toInt()
            width1 < minimumWidth -> minimumWidth.toInt()
            else -> width1
        })

        height1 = if (height1 == -1) getMinimumVideoHeight(width1.toFloat()).toInt()
        else when {
            height1 > maximumHeight -> maximumHeight.toInt()
            height1 < minimumHeight -> minimumHeight.toInt()
            else -> height1
        }

        popupLayoutParams!!.width = width1
        popupLayoutParams!!.height = height1
        popupWidth = width1.toFloat()
        popupHeight = height1.toFloat()

        Log.d(TAG, "updatePopupSize() updated values:  width = [$width1], height = [$height1]")
    ...
    inner class VideoPlayerImpl internal constructor(context: Context) : VideoPlayer("${PopupVideoPlayer.TAG}.VideoPlayerImpl", context), View.OnLayoutChangeListener {
        ...
        override fun initViews(rootView: View) {
            ...
            fullScreenButton!!.setOnClickListener { _ -> onFullScreenButtonClicked() }
        ...
        override fun initListeners() {
            super.initListeners()
            videoPlayPause!!.setOnClickListener { _ -> onPlayPause() }
        }
    ...
    companion object {
        ...
        const val ACTION_CLOSE = "com.dew.aihua.simpleExoPlayer.PopupVideoPlayer.CLOSE"
        const val ACTION_PLAY_PAUSE = "com.dew.aihua.simpleExoPlayer.PopupVideoPlayer.PLAY_PAUSE"
        const val ACTION_REPEAT = "com.dew.aihua.simpleExoPlayer.PopupVideoPlayer.REPEAT"
